<script setup lang="ts">
import { useRouter } from "vue-router";

</script>

<template>
<div class="">
    <div class="p-4  flex justify-center items-center">
    <img src="/src/assets/images/Vector (2).png" class=" " />
    </div>
    <p class="text-sm/[16px] text-center ">L’opération de reversement du compte business <b>Open SI SAS</b> a été effectué avec succès !</p>
    <div class="flex justify-center py-8">
    <div class="">
    <img src="/src/assets/images/wallet-outline.png" class="p-1" />
    </div>
    <P class="text-center text-sm/[16px] text-center ">Solde disponible <br><b>18 345 025 Fcfa </b></P>
    <P class="text-center text-sm/[16px] text-center px"></P>
    </div>
    <div class="flex justify-center space-x-4">
    <div class="rounded border-2 border-red-500 w-52 h-10  py-2 w-full flex justify-center">
    <button type="submit" class="flex justify-center items-center text-white">
        <p class="text-red-500">Terminer</p>
      
    </button>
  </div>  
  

    <div class="rounded border-1 bg-red-500  py-2 w-52 h-10 flex justify-center">
    <button type="submit" class="flex  items-center text-white">
        
    <img src="/src/assets/images/download.png" class="px-2" />
    <p>Télécharger la facture</p>
    </button>

    
  </div>
</div>
  
    
    
</div>


</template>